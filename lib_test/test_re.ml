...
let _ =
  ...
  expect_pass "Group.all" (fun () ->
    expect_eq_arr_str id [| "ab"; "a"; ""; "b" |] Group.all m);
  expect_pass "Group.all_offset" (fun () ->
    expect_eq_arr_ofs id [| 0, 2; 0, 1; -1, -1; 1, 2 |] Group.all_offset m);
  ...
  expect_pass "repn" (fun () ->
    ...
    expect_equal_app invalid_arg "Re.repn" (fun () -> repn empty (-1) None) ();
    expect_equal_app invalid_arg "Re.repn" (fun () -> repn empty 1 (Some 0)) ());
  expect_pass "opt" (fun () ->
  ...
  expect_pass "default match semantics" (fun () ->
    re_match (seq [ rep (alt [ char 'a'; char 'b' ]); char 'b' ]) "aabaab" [| 0, 6 |];
    re_match (alt [ str "aa"; str "aaa" ]) "aaaa" [| 0, 2 |];
    re_match (alt [ str "aaa"; str "aa" ]) "aaaa" [| 0, 3 |]);
  expect_pass "shortest match" (fun () ->
    re_match
      (shortest (seq [ rep (alt [ char 'a'; char 'b' ]); char 'b' ]))
      "aabaab"
      [| 0, 3 |];
    re_match (shortest (alt [ str "aa"; str "aaa" ])) "aaaa" [| 0, 2 |];
    re_match (shortest (alt [ str "aaa"; str "aa" ])) "aaaa" [| 0, 2 |]);
  expect_pass "longest match" (fun () ->
    re_match
      (longest (seq [ rep (alt [ char 'a'; char 'b' ]); char 'b' ]))
      "aabaab"
      [| 0, 6 |];
    re_match (longest (alt [ str "aa"; str "aaa" ])) "aaaa" [| 0, 3 |];
    re_match (longest (alt [ str "aaa"; str "aa" ])) "aaaa" [| 0, 3 |]);
  expect_pass "first match" (fun () ->
    re_match
      (first (seq [ rep (alt [ char 'a'; char 'b' ]); char 'b' ]))
      "aabaab"
      [| 0, 6 |];
    re_match (first (alt [ str "aa"; str "aaa" ])) "aaaa" [| 0, 2 |];
    re_match (first (alt [ str "aaa"; str "aa" ])) "aaaa" [| 0, 3 |]);
  expect_pass "greedy" (fun () ->
    re_match
      (greedy (seq [ rep (alt [ char 'a'; char 'b' ]); char 'b' ]))
      "aabaab"
      [| 0, 6 |];
    re_match (greedy (rep (group (opt (char 'a'))))) "aa" [| 0, 2; 2, 2 |]);
  expect_pass "non_greedy" (fun () ->
    ...
    re_match
      ...
      "aabaab"
      [| 0, 3 |];
    re_match (non_greedy (longest (rep (group (opt (char 'a')))))) "aa" [| 0, 2; 1, 2 |]);
  ...
  expect_pass "no_group" (fun () ->
    let r =
      no_group (seq [ group (char 'a'); opt (group (char 'a')); group (char 'b') ])
    in
    expect_eq_arr_ofs
      id
      [| 0, 2 |]
  ...
  let test ?pos msg re input expected =
    ...
    expect_pass msg (fun () ->
      expect_equal_app
        ...
        expected
        extract_groups
        (exec_partial_detailed ?pos (compile re) input)
        ~printer:(function
  ...
  *)
  ...
  test
    "exec_partial_detailed 10"
    ~pos:1
    (seq [ not_boundary; str "b" ])
    "ab"
    (`Full [| 1, 2 |]);
  test
    "exec_partial_detailed 11"
