...
let%expect_test "codec error" =
  let stream = Re.any |> Re.compile |> Stream.create in
  ignore_or_exception (fun () -> ignore (Stream.feed stream "foo" ~pos:2 ~len:3));
  [%expect {| "CodecError" |}];
  ignore_or_exception (fun () -> ignore (Stream.finalize stream "foo" ~pos:2 ~len:3));
  [%expect {| "CodecError" |}];
  let stream = Stream.Group.create stream in
  ignore_or_exception (fun () -> ignore (Stream.Group.feed stream "foo" ~pos:2 ~len:3));
  [%expect {| "CodecError" |}];
  ignore_or_exception (fun () -> ignore (Stream.Group.finalize stream "foo" ~pos:2 ~len:3));
  [%expect {| "CodecError" |}]
;;
...
let%expect_test "group - mark entire string must match" =
  let m1, f = Re.(mark (char 'f' |> letter)) in
