...
let%expect_test "empty group" =
  ...
  (* Format.printf "[Test_group.empty_group] empty: '%a'@." Re.pp empty; *)
  t empty "";
  [%expect {| <None> |}];
  t empty "x";
  [%expect {| <None> |}]
...
let%expect_test "nesting of groups" =
  let re = group (seq [ group (char 'a' |> letter); char 'b' |> letter ]) in
  t re "ab";
...
let%expect_test "group choice" =
  ...
  let lhs_group =
    let open Re in
    alt [ group (char 'a' |> letter); char 'b' |> letter ]
  in
  t lhs_group "a";
  ...
  t
    (let open Re in
     alt [ group (char 'a' |> letter); group (char 'b' |> letter) ])
    "b";
  [%expect {| `Full [|0,1,"b";-1,-1,<No match>;0,1,"b"|] |}];
  (* No_group inside char set: *)
  let no_group_charset =
    let a = Re.group (Re.char 'a' |> letter) in
    let b = Re.char 'b' |> letter in
...
let%expect_test "Group.{get,get_opt,offset,test}" =
  let r = seq [ group (char 'a' |> letter); opt (group (char 'a' |> letter)); group (char 'b' |> letter) ] in
...
let%expect_test "nest" =
  let r = rep (nest (alt [ group (char 'a' |> letter); char 'b' |> letter ])) in
  test_re r "ab";
  [%expect {| [| (0, 2); (-1, -1) |] |}];
  test_re r "ba";
  [%expect {| [| (0, 2); (1, 2) |] |}]
