...
module type T = sig
  ...
  module Group : sig
    (** Information about groups in a match. As is conventional, every match
        implicitly has a group 0 that covers the whole match, and explicit
    ...
    *)
    ...
    (** Similar to {!offset}, but returns an option instead of using an
        exception. *)
    val offset_opt : t -> int -> (int * int) option

    (** Return the start of the match. Raise [Not_found] if the group did not
        match. *)
    val start : t -> int -> int

    (** Similar to {!start_opt}, but returns an option instead of using an
        exception. *)
    val start_opt : t -> int -> int option

    (** Return the end of the match. Raise [Not_found] if the group did not
        match. *)
    val stop : t -> int -> int

    (** Similar to {!stop_opt}, but returns an option instead of using an
        exception. *)
    ...
    (** Returns the total number of groups defined - matched or not. This
  ...
  (** Compile a regular expression into an executable version that can be used
      to match strings, e.g. with {!exec}. *)
  val compile : t -> re

  (** Return the number of capture groups (including the one corresponding to
      the entire regexp). *)
  ...
  (** [exec re str] searches [str] for a match of the compiled expression [re],
      ...
      More specifically, when a match exists, [exec] returns a match that starts
      at the earliest position possible. If multiple such matches are possible,
      the one specified by the match semantics described below is returned.
  ...
  (** Similar to {!exec}, but returns [true] if the expression matches, and
      [false] if it doesn't. This function is more efficient than calling
      {!exec} or {!exec_opt} and ignoring the returned group.
  ...
  (** More detailed version of {!execp}. [`Full] is equivalent to [true], while
      [`Mismatch] and [`Partial] are equivalent to [false], but [`Partial]
  ...
  (** More detailed version of {!exec_opt}. [`Full group] is equivalent to
      [Some group], while [`Mismatch] and [`Partial _] are equivalent to [None],
      but [`Partial position] indicates that the input string could be extended
      to create a match, and no match could start in the input string before the
      given position. This could be used to not have to search the entirety of
      the input if more becomes available, and use the given position as the
      [?pos] argument. *)
  ...
  (** [split re s] splits [s] into chunks separated by [re]. It yields the
      chunks themselves, not the separator. An occurence of the separator at the
      beginning or the end of the string is ignoring.
      ...
      and [eow]. Because they don't have any width, they will still be present
      in the result. (Note the position of the [\n] and space characters in the
      output.)
  ...
  module Seq : sig
    (** Same as {!module-Re.val-all} but returns an iterator.
        ...
        @since 1.10.0 *)
    val all :
      ?pos:int (** Default: 0 *) -> ?len:int -> re -> string -> Group.t Seq.t
    ...
    (** Same as {!module-Re.val-matches}, but returns an iterator.
        ...
        @since 1.10.0 *)
    val matches :
      ?pos:int (** Default: 0 *) -> ?len:int -> re -> string -> string Seq.t
  ...
  *)
  ...
  (** [repn re i j] matches [re] at least [i] times and at most [j] times,
      bounds included. [j = None] means no upper bound. *)
  ...
  *)
  ...
  (** Beginning of string. This differs from {!start} because it matches the
      beginning of the input string even when using [~pos] arguments:
  ...
  (** End of string. This is different from {!stop} in the way described in
      {!bos}. *)
  val eos : t
  ...
  (** Initial position. This differs from {!bos} because it takes into account
      the [~pos] arguments:
  ...
  (** Final position. This is different from {!eos} in the way described in
      {!start}. *)
  val stop : t
  ...
  (** {2 Match semantics}
      ...
      By default, the leftmost branch of alternations is preferred, and
      ...
      match when applied to "ab". However if
      [seq [ bos; str "a"; non_greedy (opt (str "b")) ]] is applied to "ab", it
      will match "a" rather than "ab".

      Also note that multiple match semantics can conflict. In this case, the
      one executed earlier takes precedence. For instance, any match of
      [shortest (seq [ bos; group (rep (str "a")); group (rep (str "a")); eos
       ])] will always have an empty first group. Conversely, if we use
      [longest] instead of [shortest], the second group will always be empty. *)
  ...
  *)
  ...
  (** Greedy matches for repetitions ({!opt}, {!rep}, {!rep1}, {!repn}): they
      will match as many times as possible. *)
  val greedy : t -> t

  (** Non-greedy matches for repetitions ({!opt}, {!rep}, {!rep1}, {!repn}):
      they will match as few times as possible. *)
  val non_greedy : t -> t

  (** {2 Groups (or submatches)} *)

  (** Delimit a group. The group is considered as matching if it is used at
      least once (it may be used multiple times if is nested inside {!rep} for
  ...
  (** When matching against [nest e], only the group matching in the last match
      of e will be considered as matching.
  ...
  (** Case insensitive matching. Note that this works on latin1, not ascii and
      not utf8. *)
  val no_case : t -> t
...
struct
  module Ast = Ast.Make (Cset) (Color_map)
  module Compile = Compile.Make (Cset) (Color_map)
  module Search = Search.Make (Cset) (Color_map)
  ...
  type split_token = [ `Text of string | `Delim of Group.t ]
  ...
  module Gen = struct
    ...
    let split_full ?pos ?len re s : _ gen =
      Search.split_full ?pos ?len re s |> gen_of_seq
