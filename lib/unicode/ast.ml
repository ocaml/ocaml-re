...
let equal_ast (type a) eq (x : (a, [ `Uncased ]) ast)
  (y : (a, [ `Uncased ]) ast) =
  match (x, y) with Alternative a, Alternative b -> List.equal ~eq a b
...
let rec dyn_of_gen f =
  ...
  | Repeat (gen, min, max) ->
    let base = match max with None -> [] | Some x -> [ int x ] in
  ...
  | Group (name, t) ->
    let args =
      ...
      match name with None -> args | Some name -> string name :: args
    in
    variant "Group" args
...
let rec pp_gen pp_cset fmt t =
  ...
  | Sem_greedy (k, re) ->
    sexp fmt "Sem_greedy" (pair Automata.Rep_kind.pp pp) (k, re)
...
let rec equal cset x1 x2 =
  ...
  | Sem_greedy (rep, a), Sem_greedy (rep', a') ->
    Poly.equal rep rep' && equal cset a a'
  | _ -> false
...
module type T = sig
  ...
  type no_case = (cset_t, [ `Uncased ]) gen

  val to_dyn : t -> Dyn.t
  val pp : t Fmt.t

  val merge_sequences :
    (cset_t, [ `Uncased ]) gen list -> (cset_t, [ `Uncased ]) gen list

  val handle_case : bool -> t -> (cset_t, [ `Uncased ]) gen
  val anchored : t -> bool
  val colorize : color_map_t -> (cset_t, [ `Uncased ]) gen -> bool
  val cset : cset_t -> t
  val t_of_cset : cset -> t

  module Export : Export with type t = t and type letter = letter
end
...
struct
  ...
  let rec handle_case_cset ign_case = function
    ...
    | Cast (Alternative l) ->
      List.map ~f:(handle_case_cset ign_case) l |> Cset.union_all
    | Complement l ->
      List.map ~f:(handle_case_cset ign_case) l
      |> Cset.union_all |> Cset.diff Cset.cany
    | Difference (r, r') ->
      Cset.inter
        (handle_case_cset ign_case r)
        (Cset.diff Cset.cany (handle_case_cset ign_case r'))
    | Intersection l ->
      List.map ~f:(handle_case_cset ign_case) l |> Cset.intersect_all
    | Cast (No_case a) -> handle_case_cset true a
    | Cast (Case a) -> handle_case_cset false a

  let rec handle_case ign_case : t -> (Cset.t, [ `Uncased ]) gen =
    fun t ->
    match t with
  ...
  module Export = struct
    type nonrec t = t
    type nonrec letter = letter

    let pp = pp
    let seq : t list -> t = function [ r ] -> r | l -> Sequence l
    let epsilon : t = seq []
    let letter l = cset (Cset.csingle l)
    (* let f =
        Dense_map.make ~size:256 ~f:(fun i ->
          cset
            (Cset.csingle (Cset.CodePage.of_int i |> Cset.CodePage.to_letter)))
      in
      fun c -> f (Cset.CodePage.from_letter c |> Cset.CodePage.to_int) *)

    let any = cset Cset.cany
    let char c = Cset.CodePage.of_char c
    let int n = Cset.CodePage.to_letter @@ Cset.CodePage.of_int n

    (* let pp_str ppf l = Format.pp_print_iter
      ~pp_sep:(fun fmt () -> Format.fprintf fmt ",@ ")
      Stdlib.List.iter
      (fun fmt letter -> Format.fprintf fmt "%a" Cset.Codec.pp letter)
      ppf l *)

    let str s : t =
      Cset.Codec.to_list s
      |> fun l ->
        List.map l ~f:letter
      |> seq
    ...
    let as_set_elems elems =
      match
        List.map elems ~f:(function Set e -> e | _ -> raise_notrace Exit)
      with
      | exception Exit -> None
      | e -> Some e
    ...
    let make_set f t =
      match t with Set x -> Set (Cast (f.f x)) | _ -> Ast (f.f t)

    let preserve_set f : t -> t = fun t -> match t with Set _ -> t | _ -> f t
    ...
    let as_set_or_error name elems =
      match as_set_elems elems with None -> invalid_arg name | Some s -> s
    ...
    let witness t =
      let rec witness (t : no_case) =
        match t with
        | Set c ->
          Cset.CodePage.to_letter (Cset.pick c)
          |> Cset.Codec.to_bytes |> Bytes.unsafe_to_string
        ...
        | Sem_greedy (_, r) | Sem (_, r) | Nest r | Pmark (_, r) | Group (_, r)
          ->
          witness r
        | Beg_of_line | End_of_line | Beg_of_word | End_of_word | Not_bound
        | Beg_of_str | Last_end_of_line | Start | Stop | End_of_str ->
          ""
      in
  ...
  (* open Export *)
  let seq = function [ r ] -> r | l -> Sequence l
  ...
  let colorize color_map (regexp : no_case) =
    ...
    let rec colorize regexp =
      ...
      | Beg_of_word | End_of_word | Not_bound ->
        Color_map.split color_map Cset.cword
  ...
  and anchored : t -> bool = function
    ...
    | Pmark (_, r) ->
      anchored r
    | Set _ | Beg_of_line | End_of_line | Beg_of_word | End_of_word | Not_bound
    | End_of_str | Last_end_of_line | Stop ->
      false
