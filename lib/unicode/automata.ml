...
module Rep_kind = struct
  ...
  let to_string = function `Greedy -> "Greedy" | `Non_greedy -> "Non_greedy"
  let to_dyn t = Dyn.enum (to_string t)
...
module Marks = struct
  type t = { marks : (Mark.t * Idx.t) list; pmarks : Pmark.Set.t }
  ...
  let to_dyn { marks; pmarks } : Dyn.t =
    ...
    record
      [
        ( "marks",
          List.map marks ~f:(fun (m, idx) ->
            pair (Mark.to_dyn m) (Idx.to_dyn idx))
          |> list );
  ...
  let marks_set_idx =
    let rec marks_set_idx idx marks =
      ...
      | [] -> []
      | (a, idx') :: rem ->
        if Idx.equal idx' Idx.unknown then (a, idx) :: marks_set_idx idx rem
        else marks
    in
...
module Make (Cset : Cset.T) = struct
  ...
  module Expr = struct
    ...
    let wrap_sem sem sem' v =
      ...
      | None | Some _ -> (
        match v with List v -> variant name v | _ -> variant name [ v ])

    let rec seq_as_list sem = function
      | Eps -> []
      | Cst cs -> [ Cst cs ]
      | Seq (sem', x, y) ->
        if Sem.equal sem sem' then x.def :: seq_as_list sem y.def
        else raise_notrace Not_found
      | _ -> raise_notrace Not_found

    let seq_as_list sem t =
      match seq_as_list sem t with exception Not_found -> None | s -> Some s
    ...
    let rec dyn_of_def sem =
      ...
      | Eps -> Enum "Eps"
      | Rep (_, sem', t) ->
        wrap_sem sem sem' (variant "Rep" [ to_dyn (Some sem') t ])
    ...
    let rec pp_with_sem sem ch e =
      ...
      | Seq (k, e, e') ->
        sexp ch "seq"
          (triple Sem.pp (pp_with_sem sem) (pp_with_sem sem))
          (k, e, e')
      | Eps -> str ch "eps"
      | Rep (_rk, k, e) ->
        sexp ch "rep" (pair Sem.pp (pp_with_sem (Some k))) (k, e)
    ...
    let alt ids = function [] -> empty ids | [ c ] -> c | l -> mk ids (Alt l)
    ...
    let is_eps expr = match expr.def with Eps -> true | _ -> false
  ...
  module Desc : sig
    ...
    module E = struct
      ...
      and equal x y =
        ...
        | TSeq (_, l1, e1), TSeq (_, l2, e2) ->
          Id.equal e1.id e2.id && equal_list l1 l2
      ...
      let rec hash (t : t) accu =
        match t with
        | TSeq (_, l, e) ->
          hash_combine 0x172a1bce
            (hash_combine (Id.hash e.id) (hash_list l accu))
        | TExp (marks, e) ->
          hash_combine 0x2b4c0d77
            (hash_combine (Id.hash e.id) (Marks.hash marks accu))
    ...
    let rec fold_right t ~init ~f =
      match t with [] -> init | x :: xs -> f x (fold_right xs ~init ~f)
    ...
    let rec print_state_rec ch e (y : Expr.t) =
      ...
      | TExp (marks, { def = Eps; _ }) ->
        Format.fprintf ch "@[<2>(TExp@ %a@ (%a)@ (eps))@]" Id.pp y.id Marks.pp
          marks
      | TExp (marks, x) ->
        Format.fprintf ch "@[<2>(TExp@ %a@ (%a)@ %a)@]" Id.pp x.id Marks.pp
          marks Expr.pp x
    ...
    let remove_matches =
      List.filter ~f:(function TMatch _ -> false | _ -> true)
    ...
    let status : _ -> Status.t = function
      | [] -> Failed
      | TMatch m :: _ ->
        Match (Mark_infos.make (m.marks :> (int * int) list), m.pmarks)
      | _ -> Running
    ...
    let[@ocaml.warning "-32"] pp fmt t =
      Format.fprintf fmt "[%a]"
        (Format.pp_print_list ~pp_sep:(Fmt.lit "; ") pp)
        t
    ...
    let remove_duplicates =
      let rec loop seen l y =
        ...
        | (TExp (_marks, { def = Eps; _ }) as e) :: r ->
          if Id.Hash_set.mem seen y.id then loop seen r y
          else (
            Id.Hash_set.add seen y.id;
            e :: loop seen r y)
        | (TExp (_marks, x) as e) :: r ->
          if Id.Hash_set.mem seen x.id then loop seen r y
          else (
  ...
  module State = struct
    ...
    let equal { idx; category; desc; status = _; hash } t =
      Int.equal hash t.hash && Idx.equal idx t.idx
  ...
  module Working_area = struct
    ...
    let rec find_free tbl idx len =
      if idx = len || not (Bit_vector.get tbl idx) then idx
      else find_free tbl (idx + 1) len
  ...
  type ctx = { cp : Cset.cp; prev_cat : Category.t; next_cat : Category.t }

  let rec delta_expr ({ cp; _ } as ctx) marks (x : Expr.t) rem =
    (*Format.eprintf "%d@." x.id;*)
    match x.def with
    | Cst s -> if Cset.mem cp s then Desc.add_eps rem marks else rem
    ...
    | Before cat ->
      if Category.intersect ctx.next_cat cat then Desc.add_match rem marks
      else rem
    | After cat ->
      if Category.intersect ctx.prev_cat cat then Desc.add_match rem marks
      else rem
  ...
  and delta_alt ctx marks l rem =
    List.fold_right l ~init:rem ~f:(delta_expr ctx marks)
  ...
  and delta_seq ctx (kind : Sem.t) y z rem =
    ...
    | Some marks -> (
      match kind with
      | `Longest ->
        Desc.tseq kind (Desc.remove_matches y) z (delta_expr ctx marks z rem)
      | `Shortest ->
        delta_expr ctx marks z (Desc.tseq kind (Desc.remove_matches y) z rem)
      | `First ->
  ...
  let delta (tbl_ref : Working_area.t) next_cat letter (st : State.t) =
    let expr =
      let prev_cat = st.category in
      let ctx = { cp = letter; next_cat; prev_cat } in
      Desc.remove_duplicates tbl_ref.seen
