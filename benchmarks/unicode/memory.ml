...
let re () =
  let open Re_unicode.Utf8.Re in
  compile @@ seq [ rep (set "01"); char '1' |> letter; repn (set "01") size (Some size) ]
;;

(* Another pathological case that is a simplified version of the above *)
let re2 () =
  let open Re_unicode.Utf8.Re in
  seq [ rep (set "01"); char '1' |> letter; repn (set "01") size (Some size); char 'x' |> letter ] |> compile
;;
  ...
  |> ListLabels.map ~f:(fun (name, re) ->
    Bench.Test.create_indexed ~name ~args:[ 10; 20; 40; 80; 100; size ] (fun len ->
      Staged.stage (fun () ->
        ...
        ignore (Re_unicode.Utf8.Re.execp ~pos:0 ~len re str))))
;;
