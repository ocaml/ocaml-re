open Core
open Core_bench
open Re_unicode.Utf8

let str_20_zeroes = String.make 20 '0'
let re_20_zeroes = Re.(str str_20_zeroes)
let lots_of_a's = String.init 101 ~f:(function 100 -> 'b' | _ -> 'a')

let lots_o_a's_re =
  Re.(seq [ char 'a' |> letter; opt (char 'a' |> letter); char 'b' |> letter ])
  ...
  Bench.Test.create_group ~name
    (List.concat_map cases ~f:(fun data ->
       let name =
         ...
         if len > 70 then
           Printf.sprintf "%s .. (%d)" (String.sub data ~pos:0 ~len:10) len
         else data
       in
...
let compile_clean_star =
  ...
  let re = Re.rep (Re.char 'c' |> Re.letter) in
  let re () = Re.compile re in
  test ~name:"kleene star compilation" re (fun re ->
...
let gen_chars n =
  ...
  for i = 0 to n - 1 do
    pos :=
      !pos
      + Cset.Codec.set bytes !pos (Cset.CodePage.of_char @@ Stdlib.Char.chr i)
  done;
  Stdlib.Bytes.sub_string bytes 0 !pos

let repeated_sequence =
  let s = gen_chars 256 in
...
let split =
  ...
  test ~name:"split on whitespace" re (fun re ->
    ignore (Re.split_full (re ()) s))
...
let prefixes =
  ...
  let re () =
    ...
    List.init 100 ~f:(fun i ->
      ...
      let open Re in
      seq [ rep1 any; char '.' |> letter; str ext ])
    |> Re.alt |> Re.compile
  in
...
let benchmarks =
  ...
  @ repeated_sequence @ split @ prefixes
